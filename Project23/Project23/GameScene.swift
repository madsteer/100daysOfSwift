...
class GameScene: SKScene {
    ...
    func createScore() {
        ...
        gameScore.position = CGPoint(x: 10, y: 35)
        score = 0
    }
    ...
    func createLives() {
        for i in 0 ..< 3 {
            ...
            spriteNode.position = CGPoint(x: CGFloat(834 + (i * 70)), y: 708)
            addChild(spriteNode)
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for case let node as SKSpriteNode in nodesAtPoint {
            if node.name == "enemy" {
                ...
                // destroy bomb
                guard let bomContainer = node.parent as? SKSpriteNode else {
                    continue
                }

                if let emitter = SKEmitterNode(fileNamed: "sliceHitBom") {
                    emitter.position = bomContainer.position
                    addChild(emitter)
                }

                node.name = ""
                bomContainer.physicsBody?.isDynamic = false
                let scaleOut = SKAction.scale(to: 0.001, duration: 0.2)
                let fadeOut = SKAction.fadeOut(withDuration: 0.2)
                let group = SKAction.group([scaleOut, fadeOut])

                let seq = SKAction.sequence([group, .removeFromParent()])
                bomContainer.run(seq)

                if let index = activeEnemies.firstIndex(of: bomContainer) {
                    activeEnemies.remove(at: index)
                }
